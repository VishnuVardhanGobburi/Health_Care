...
def render():
    ...
    st.markdown("Establishing trust in the data before drawing conclusions.")
    try:
        df = _load_data()
    ...
    st.subheader("Schema and data types")
    ...
    st.subheader("Category standardization")
    st.caption("Values for sex, smoker, region (after lower + strip):")
    ...
    st.subheader("Charges (target): spread and distribution")
    ...
    st.markdown("*Takeaway: Age, BMI, and children show typical spreads; outliers are worth reviewing, not removing.*")
